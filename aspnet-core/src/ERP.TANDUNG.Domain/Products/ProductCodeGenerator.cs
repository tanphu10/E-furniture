...
namespace ERP.TANDUNG.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(TANDUNGConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(TANDUNGConsts.ProductIdentitySettingId, "Sản phẩm", TANDUNGConsts.ProductIdentitySettingPrefix, 1, 1));
